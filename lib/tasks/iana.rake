...
namespace :iana do
  ...
  task :refresh => :environment do
    ...
    Update.execute(registry) if Download.execute(registry)
  end
...
=end
  ...
  module Update
    ...
    # 戻り値 :: 必ず *true* を返します。
    def self.clear()
      Statistics::Record.delete_all
      Statistics::Version.delete_all
      Statistics::Summary.delete_all
      return true
    end
    ...
    def self.set_version(registry, data = {})
      ...
      version = Statistics::Version.new(row)
    ...
    def self.set_summary(registry, data = {})
      ...
      summary = Statistics::Summary.new(row)
    ...
    def self.set_record(registry, data = {})
      ...
      record = Statistics::Record.new(row)
      if record.save then
      end
    end
    ...
    def self.conv_addr(type, addr, value)
      ...
      if addr.length > 0 then
        ...
        when IPV4
          ...
          row[:start_addr_dec] = StringConvertConcern.zero_fill_number_string(saddr.to_range.first.to_i)
          row[:end_addr_dec] = StringConvertConcern.zero_fill_number_string(saddr.to_range.last.to_i)
        when IPV6
        ...
        when ASN
          ...
          row[:start_addr_dec] = StringConvertConcern.zero_fill_number_string(addr.to_i)
          row[:end_addr_dec] = StringConvertConcern.zero_fill_number_string(row[:end_addr].to_i)
        end
      end
      return row
    end
